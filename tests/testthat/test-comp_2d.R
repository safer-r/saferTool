test_that("comp_2d function works correctly", {
  ...
  test_that("handles large matrices with the same content", {
    expect_no_error(comp_2d(mat1, mat2))
  })

  # Test large matrices with different content
  test_that("handles large matrices with different content", {
    expect_no_error(comp_2d(mat1, mat3))
  })

  # Test matrices with the same row content and same row names
  test_that("handles matrices with the same row content and same row names", {
    expect_no_error(comp_2d(mat4, mat5))
  })

  # Test matrices with the same row content but different row names
  test_that("handles matrices with the same row content but different row names", {
    expect_no_error(comp_2d(mat4, mat6))
  })

  # Test data frames with the same row content and same row names, not same mode between columns
  test_that("handles data frames with the same row content and same row names, not same mode between columns", {
    dataframe1[, 5] <- as.character(dataframe1[, 5])
    dataframe2[, 5] <- as.character(dataframe2[, 5])
    expect_no_error(comp_2d(dataframe1, dataframe2))
  })

  # Test data frames with the same row content but different row names
  test_that("handles data frames with the same row content but different row names", {
    dataframe1[, 5] <- as.character(dataframe1[, 5])
    dataframe3[, 5] <- as.character(dataframe3[, 5])
    expect_no_error(comp_2d(dataframe1, dataframe3))
  })
})

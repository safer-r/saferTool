test_that("round2 function works correctly", {
    ...
    options(digits = 8)
    expect_equal(round2(data = vec2, safer_check = TRUE), c(NA, 10, 100.001, 333.000130, 12312.120000))
    options(digits = ini_options)
    
    ini_options <- options()$digits
    options(digits = 8)
    expect_equal(round2(data = vec3, dec.nb = 2, after.lead.zero = FALSE, safer_check = TRUE), c(NA, "10", "100.00", "333.00", "12312.12"))
    options(digits = ini_options)
})